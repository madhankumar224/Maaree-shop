...
export default function Navbar() {
  const { user, logout, cartCount, wishlist, avatar, openProductModal, openProfileDrawer, notifications, newCount, markNotificationsViewed, deleteNotification } = useApp();
              ...
              <div className="relative" ref={notifRef}>
                <button
                  onClick={() => {
                    ...
                    if (!wasOpen && newCount > 0) markNotificationsViewed();
                  }}
                ...
                >
                  ...
                  {newCount > 0 && (
                    <span className="absolute top-0.5 right-0.5 bg-red-500 text-white text-[9px] font-bold rounded-full h-[16px] min-w-[16px] flex items-center justify-center px-1 border-2 border-white">
                      {newCount}
                    </span>
                  )}
                  {!notifOpen && (
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((notif) => (
                          <div
                            key={notif.id}
                            className={`px-5 py-3 border-b border-warm-border/50 last:border-b-0 transition group/item ${
                              notif.status === "new" ? "bg-terracotta/5" :
                              notif.status === "viewed" ? "bg-warm-bg/30" : "hover:bg-warm-bg/50"
                            }`}
                          >
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold uppercase tracking-wider shrink-0 ${
                                    notif.status === "new" ? "bg-terracotta/15 text-terracotta" :
                                    notif.status === "viewed" ? "bg-sage/15 text-sage" :
                                    "bg-warm-border/60 text-warm-muted"
                                  }`}>
                                    {notif.status === "new" ? "New" : notif.status === "viewed" ? "Viewed" : "Past"}
                                  </span>
                                </div>
                              ...
                              <button
                                ...
                                className="opacity-0 group-hover/item:opacity-100 text-warm-muted hover:text-blush transition-all shrink-0 mt-0.5 p-1 rounded-md hover:bg-blush/10"
          ...
          <div className="md:hidden pb-4 space-y-1 border-t border-warm-border pt-3">
            ...
            {user && (
              <button
                ...
                onClick={() => { setNotifOpen(!notifOpen); if (!notifOpen && newCount > 0) markNotificationsViewed(); }}
              >
                Notifications
                {newCount > 0 && (
